...
简化分子线性输入规范（Simplified molecular-input line-entry system, SMILES）是一种用 ASCII 字符串明确描述分子结构的规范。它将复杂的二维或三维化学结构转化为易于阅读和存储的纯文本形式，被广泛应用于化学数据库搜索、分子编辑软件及化学信息学中。
...
> SMILES 渲染基于 [Fuyeor/chemistry](https://github.com/Fuyeor/chemistry) 公共 API，它是基于 RDKit 封装的。
