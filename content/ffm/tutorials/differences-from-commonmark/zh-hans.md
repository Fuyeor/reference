...
Markdown 作为轻量级标记语言，衍生出了多种不同的变体（Flavored Markdown）。例如最普及的 Common Flavored Markdown（CommonMark），或 GitHub 在前者基础上扩展而来的 GitHub Flavored Markdown（GFM）等。

## 与 CommonMark 的差异

FFM 和 CommonMark 是两种语法有交集但在设计哲学上不同的 Markdown 规范。
...
FFM 支持 GFM 的表格、任务列表语法，但不支持 GitHub 风格删除线（`~~text~~`）。
